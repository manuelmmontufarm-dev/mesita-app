import { test, expect } from "@playwright/test";

import { enterTable, openDevice, resetDemoTable } from "./helpers/demo";

const TOKEN = "demo-mesa-1";

test.beforeEach(async ({ request }) => {
  await resetDemoTable(request, TOKEN);
});

test.describe("demo mesa-1 — almuerzo clásico (empty start)", () => {
  test("2 devices enter and see Mesa 1 menu", async ({ browser }) => {
    const a = await openDevice(browser);
    const b = await openDevice(browser);
    try {
      await enterTable(a.page, TOKEN);
      await enterTable(b.page, TOKEN);

      await expect(a.page.getByText("Mesa 1", { exact: false })).toBeVisible();
      await expect(a.page.getByText(/Bolón|Churrasco|Llapingachos/i).first()).toBeVisible();
      await expect(b.page.getByText(/Bolón|Churrasco|Llapingachos/i).first()).toBeVisible();
    } finally {
      await a.ctx.close();
      await b.ctx.close();
    }
  });
});
